...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (session?.user) {
      ...
      const userData = {
        id, email: session.user.email || '', name: session.user.name || null,
        image: session.user.image || null, role, phone: null, provider, pinChangeRequired,
      };
      setUser(userData);
  ...
  useEffect(() => {
    ...
    if (currentView === 'passenger') setCurrentView('passenger-login');
    else if (currentView === 'coordinator') setCurrentView('coordinator-login');
    else if (currentView === 'driver') setCurrentView('driver-login');
  }, [currentView, isAuthenticated, initialized, setCurrentView]);

  const renderView = () => {
    switch (currentView) {
      case 'landing': return <LandingView />;
      case 'passenger-login': return <PassengerLoginPage />;
      case 'coordinator-login': return <CoordinatorLoginPage />;
      case 'driver-login': return <DriverLoginPage />;
      case 'passenger': return <AuthGuard allowedRoles={['passenger']}><PassengerPortal /></AuthGuard>;
      case 'coordinator': return <AuthGuard allowedRoles={['coordinator']}><CoordinatorDashboard /></AuthGuard>;
      case 'driver': return <AuthGuard allowedRoles={['driver']}><DriverInterface /></AuthGuard>;
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        <AnimatePresence mode="wait">
          <motion.div key={currentView} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}>
            {renderView()}
          </motion.div>
        </AnimatePresence>
      </main>
